...
namespace ShowDialogs
{
    public static class LCB_Voice
    {
        ...
        {
            if (!_unitView || !path.StartsWith(VoiceGenerator.VOICE_EVENT_PATH + "battle_"))
                return;
            path = path[VoiceGenerator.VOICE_EVENT_PATH.Length..];
            if (path.Contains("devai"))
                CallForPoluAsync(path, _unitView);
    ...
    BattleSkillViewer skillViewer)
        ...
        public static async void CallForPoluAsync(string path, BattleUnitView view)
        {
        ...
        public static List<string> lookingfordlg(string search)
        {
        ...
        public static List<int> lookingfortime(string search)
        {
            var lookingfor1 = devyatdata.dataList.FirstOrDefault(f => f.id == search);
            if (lookingfor1 != null)
            {
