...
namespace LimbusLocalizeRUS
{
    internal class LCBR_TextUI
    {
        ...
        private static void TicketChangePopup(UserInfoTicketItem __instance)
        {
            ...
            __instance._using_choosingText.lineSpacing = -40;
        }
        #endregion
        ...
        private static void FormationLabel_Init(FormationPersonalityUI __instance)
        {
            ...
            __instance._label.tmp_text.text = "<voffset=-0.25em><cspace=-1px>Замена</cspace></voffset>";
            __instance._label.tmp_text.m_fontAsset = LCB_Cyrillic_Font.GetCyrillicFonts(0);
            __instance._label.tmp_text.m_sharedMaterial = LCB_Cyrillic_Font.GetCyrillicMats(2);
            //хз когда замена приходит, пусть пока останется
        }
        ...
        static string declension_sinners(string sinnerName, string replacing)
        {
            switch (sinnerName)
            {
                case "Faust" or "Donqui" or "Ryoshu" or "Ishmael" or "Rodion" or "Outis":
        ...
        {
            __instance.tmp_tabName.text = __instance.tmp_tabName.text.Replace("Атака", "Атаки");
        }
        ...
        {
            __instance.tmp_storyButton.lineSpacing = -40;
        }
        ...
        [HarmonyPostfix]
        private static void StoryIntroduceCharacterDescription_SetData(StoryIntroduceCharacterDescription __instance)
        {
            foreach (var lala in __instance._textList)
            {
                lala.fontSharedMaterial = LCB_Cyrillic_Font.GetCyrillicMats(14);
                Color wowie = new Color(lala.color.r, lala.color.g, lala.color.b, 0.25f);
                lala.fontSharedMaterial.SetFloat("_GlowInner", 0.4f);
                lala.fontSharedMaterial.SetFloat("_GlowOuter", 0.6f);
                lala.fontSharedMaterial.SetFloat("_GlowPower", 3);
                lala.fontSharedMaterial.SetColor("_GlowColor", wowie);
            }
        }
        [HarmonyPatch(typeof(StageInfoWaveButtonListUI), nameof(StageInfoWaveButtonListUI.SetData))]
        [HarmonyPostfix]
        private static void StageInfoWaveButtonListUI_SetData(StageInfoWaveButtonListUI __instance)
        {
            foreach (var lala in __instance._waveButtons)
            {
                lala.tmp_buttonText.m_fontAsset = LCB_Cyrillic_Font.GetCyrillicFonts(0);
                lala.tmp_buttonText.m_sharedMaterial = LCB_Cyrillic_Font.GetCyrillicMats(2);
                lala.tmp_buttonText.fontSize = 30;
                var myN = Int32.Parse(lala.tmp_buttonText.text.Substring(5));
                lala.tmp_buttonText.text = myN + GetOrdinal(myN) + "ВОЛНА";
            }
        }
        ...
        {
            foreach(var lala in __instance._danteAbilitySlotList)
            {
                if(lala._danteAbilityModel._classInfo._sepira == SEPIRA.HOKMA)
                {
                    lala._danteAbilityModel._classInfo.name = "yourtext";
                }
            }
        }
        ...
        {
            __instance._dialogCon.tmp_name.lineSpacing = -33;
        }
        ...
        private static void MOWE_MainEvent(MOWEEventUIPanel __instance)
        {
            ...
            //UpdateButtonState, SetStory, SetStage, Initialize, Initalways - NOT WORKING
            foreach(var haha in __instance._logoImages)
            {
                if(haha.sprite.name == "MainUI_MOWE_07_26")
                {
                    haha.sprite = LCBR_ReadmeManager.ReadmeEventSprites["MOWE_Logo_Blood"];
                }
                else
                {
                    haha.sprite = LCBR_ReadmeManager.ReadmeEventSprites["MOWE_Logo"];
                }
            }
        }
    }
